...
std::string testSplitAndMerge(void *array[MAX_ALLOC]) {
    ...
    sfree(array[0]);
    checkStats(0, 0, __LINE__);
    DO_MALLOC(tmp = smalloc(small_part_of_block));
    checkStats(0, 0, __LINE__);
    sfree(array[3]);
    checkStats(0, 0, __LINE__);
    sfree(array[6]);
    checkStats(0, 0, __LINE__);
    DO_MALLOC(tmp =srealloc(array[4], default_block_size + small_part_of_block));
    checkStats(0, 0, __LINE__);
    DO_MALLOC(tmp =srealloc(array[5], default_block_size + small_part_of_block));
    checkStats(0, 0, __LINE__);
    sfree(array[10]);
    checkStats(0, 0, __LINE__);
    DO_MALLOC(tmp =srealloc(array[9], default_block_size + small_part_of_block));
    checkStats(0, 0, __LINE__);

    sfree(array[12]);
    checkStats(0, 0, __LINE__);
    DO_MALLOC(tmp =srealloc(array[13], default_block_size + small_part_of_block));
    checkStats(0, 0, __LINE__);
...
//		std::cout << "alloced blocks: " << current_stats.num_allocated_blocks << endl;
//		std::cout << "free blocks: " << current_stats.num_allocated_blocks << endl;
    }
    checkStats(0, 0, __LINE__);
...
std::string testInit(void *array[MAX_ALLOC]) {
    ...
    if (sizeof(MallocMetadata) != _size_meta_data()) {
